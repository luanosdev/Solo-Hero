...
-- local PlayerManager = require("src.managers.player_manager") -- Para ações futuras
-- local InventoryManager = require("src.managers.inventory_manager") -- Para ações futuras
...
function ItemDetailsModal:draw()
    ...
    local w = modalWidth -- Usar largura do modal para cálculos internos
    local x = modalX -- Usar X do modal para cálculos internos
    ...
    love.graphics.printf(self.item.iconPlaceholder or string.sub(self.item.name or "?", 1, 1), iconX, currentY + iconSize * 0.25, iconSize, "center")
