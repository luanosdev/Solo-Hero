...
function ItemTooltip.draw(item, baseItemData, x, y, playerStats, equippedItem)
    ...
    -- <<< NOVO: Calcular larguras das colunas para Ícone e Texto Principal >>>
    ...
    local damageFont = fonts.getFittingBoldFont(damageDisplay, textColumnWidth - PADDING * 2, LINE_HEIGHT_DAMAGE * 1.3,
        42, 28) or fonts.title_large
    table.insert(mainSectionTextLines, {
        text = damageDisplay,
        font = damageFont,
    ...
    if baseItemData.type == "weapon" then
        local attacksPerSecond = item.attacksPerSecond or baseItemData.cooldown
    ...
    local mainSectionHeight = math.max(mainSectionTextHeight, ICON_SIZE) +
        PADDING *
        2 -- Adiciona padding interno para a seção
    currentYInternal = mainSectionContentStartY + mainSectionHeight
    local mainSectionEndY =
        currentYInternal -- Guardar Y onde a seção principal (ícone + texto ao lado) termina
    ...
    for lineIdx = #tooltipLines, 1, -1 do -- Itera para encontrar linhas que não são da seção principal ou nome
        ...
        if not lineInfo.is_main_section_text and lineInfo.text and lineInfo.font and (lineInfo.alignment ~= "center" or lineIdx == 1) then
            ...
            if lineInfo.type ~= "description_marker_start" and lineInfo.type ~= "description_marker_end" then
                totalHeight = totalHeight + lineInfo.height
            end
        end
        if lineInfo.text and lineInfo.font then
            local textW = (lineInfo.font):getWidth(lineInfo.text)
            local currentLineW = (lineInfo.x_offset or 0) + textW + PADDING * 2
            if lineInfo.is_main_section_text then
                currentLineW = textColumnXOffset + (lineInfo.x_offset_in_column or 0) + textW + PADDING
            end
            if lineInfo.alignment == "center" then currentLineW = textW + PADDING * 2 end
