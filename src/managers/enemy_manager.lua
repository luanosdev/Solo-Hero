...
--- Cria uma nova instância do EnemyManager.
--- O construtor é responsável por inicializar todos os subsistemas essenciais.
---@param playerManager PlayerManager
---@param dropManager DropManager
---@return EnemyManager
function EnemyManager:new(playerManager, dropManager)
    local instance = setmetatable({}, EnemyManager)

    -- Injeta dependências essenciais
    instance.playerManager = playerManager
    instance.dropManager = dropManager
    instance.mapManager = nil -- Será injetado depois pela cena

    -- Inicializa o estado da instância
    instance.enemies = {}
    instance.maxEnemies = EnemyManager.MAX_ENEMIES
    instance.enemyPool = {}
    instance.nextEnemyId = 1
    instance.gameTimer = 0
    instance.lastOptimizationTime = 0
    instance.lastBossDeathTime = 0
    instance.bossDeathTimer = 0
    instance.bossDeathDuration = 3

    -- O Grid Espacial e os controllers agora serão criados em uma função setup.
    instance.spatialGrid = nil
    instance.spawnController = nil
    instance.despawnController = nil

    return instance
end

--- Completa a configuração do manager com dependências que só existem na cena (como o MapManager).
--- Deve ser chamado pela GameplayScene antes de setupGameplay.
---@param mapManager InfinityWrapMapManager
function EnemyManager:setupSceneDependencies(mapManager)
    self.mapManager = mapManager

    -- Agora criamos os subsistemas que dependem do mapManager
    local mapWidthInTiles, mapHeightInTiles = self.mapManager:getWorldTileDimensions()
    local tilePixelWidth, tilePixelHeight = self.mapManager.tileWidth, self.mapManager.tileHeight
    ...
    Logger.info(
        "enemy_manager.setupSceneDependencies.spatialGridCreated",
        "SpatialGrid criado na instância do EnemyManager."
    )

    -- Agora que o grid existe, podemos configurar os controllers que dependem dele.
...
--- Configura a sessão de gameplay com dados específicos da horda.
--- Assume que a instância já foi criada com todas as suas dependências.
---@param config table Tabela de configuração contendo { hordeConfig }
function EnemyManager:setupGameplay(config)
    Logger.info("enemy_manager.lifecycle.setupGameplay", "[LIFECYCLE] EnemyManager:setupGameplay CALLED.")

    if not config or not config.hordeConfig then
        error("ERRO CRÍTICO [EnemyManager:setupGameplay]: Configuração de horda ausente.")
    end

    -- Apenas configura a horda no spawnController, que já existe.
...
Logger.info("enemy_manager.lifecycle.created", "[LIFECYCLE] EnemyManager module loaded (Singleton created).")
