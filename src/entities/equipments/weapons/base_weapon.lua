...
function BaseWeapon:equip(playerManager, itemData)
    ...
    if not baseData then
        error(string.format("BaseWeapon:equip - Falha ao obter dados base para %s", self.itemBaseId))
        return
    end
    ...
    self.attackInstance = AttackClass:new(playerManager, self, ProjectileClass)
    if not self.attackInstance then
        error(string.format("BaseWeapon:equip - Falha ao criar instância de '%s'", baseData.attackClass))
        return
    end

    Logger.debug("[BaseWeapon:equip]", string.format(" '%s' equipado. Instância de ataque '%s' criada.", self.name,
