...
function GenericFlameStream:equip(playerManager, itemData)
    ...
    Logger.debug("[GenericFlameStream:equip]", string.format(" Equipping '%s'. Calling base equip done.", self.name or self.itemBaseId))
    ...
    if not baseData.attackClass then
        error(string.format("[GenericFlameStream:equip] - 'attackClass' não definido nos dados base para %s", self.itemBaseId))
        return
    end

    Logger.debug("[GenericFlameStream:equip]", string.format("  - attackClass found: %s. Attempting to load...", baseData.attackClass))
    ...
    if not success or not AttackClass then
        error(string.format("[GenericFlameStream:equip] - Falha ao carregar AttackClass '%s'. Erro: %s", baseData.attackClass,
    ...
    if not self.attackInstance then
        error(string.format("[GenericFlameStream:equip] - Falha ao criar instância de AttackClass '%s'.", baseData.attackClass))
        return
    end
    Logger.debug("[GenericFlameStream:equip]", string.format("  - Attack instance created (Type: %s).", type(self.attackInstance)))
