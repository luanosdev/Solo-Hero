...
local ArrowProjectileE001 = setmetatable({}, { __index = BaseWeapon })
ArrowProjectileE001.__index = ArrowProjectileE001 -- Herança

--- Cria uma nova instância do Arco.
---@param config table Tabela de configuração, deve conter 'itemBaseId'.
---@return ArrowProjectileE001
function ArrowProjectileE001:new(config)
...
function ArrowProjectileE001:equip(playerManager, itemData)
...
--- Desequipa o Arco.
function ArrowProjectileE001:unequip()
    print(string.format("[ArrowProjectileE001:unequip] Unequipping '%s'.", self.name or self.itemBaseId))
    BaseWeapon.unequip(self)
    print(string.format("[ArrowProjectileE001:unequip] '%s' fully unequipped.", self.name or self.itemBaseId))
end
