local BaseWeapon = require("src.items.weapons.base_weapon")

---@class AlternatingConeStrikeE001 : BaseWeapon
local AlternatingConeStrikeE001 = setmetatable({}, { __index = BaseWeapon })
AlternatingConeStrikeE001.__index = AlternatingConeStrikeE001 -- Herança

--- Cria uma nova instância das Adagas Duplas.
---@param config table Tabela de configuração, deve conter 'itemBaseId'.
---@return AlternatingConeStrikeE001
function AlternatingConeStrikeE001:new(config)
    local o = BaseWeapon.new(self, config)
    print(string.format("[AlternatingConeStrikeE001:new] Instance created, inheriting from BaseWeapon. Name: %s",
        o.name or o.itemBaseId))
    o.previewColor = { 0.8, 0.1, 0.8, 0.2 }
    o.attackColor = { 0.8, 0.1, 0.8, 0.7 }
    print("  - Default preview/attack colors set.")
    return o
end

--- Equipa as Adagas Duplas.
--- Cria a instância da lógica de ataque. A aplicação de stats agora é feita
--- pela lógica de ataque buscando os valores dinamicamente.
---@param playerManager PlayerManager Instância do PlayerManager.
---@param itemData table Dados da instância específica do item sendo equipado.
function AlternatingConeStrikeE001:equip(playerManager, itemData)
    BaseWeapon.equip(self, playerManager, itemData)
    print(string.format("[AlternatingConeStrikeE001:equip] Equipping '%s'. Calling base equip done.", self.name or self
        .itemBaseId))

    local baseData = self:getBaseData()
    if not baseData then
        error(string.format("AlternatingConeStrikeE001:equip - Falha ao obter dados base para %s", self.itemBaseId))
        return
    end
    print("  - Base data retrieved successfully.")

    if not baseData.attackClass then
        error(string.format("AlternatingConeStrikeE001:equip - 'attackClass' não definido nos dados base para %s",
            self.itemBaseId))
        return
    end
    print(string.format("  - attackClass found: %s. Attempting to load...", baseData.attackClass))

    local success, AttackClass = pcall(require, baseData.attackClass)
    if not success or not AttackClass then
        error(string.format("AlternatingConeStrikeE001:equip - Falha ao carregar AttackClass '%s'. Erro: %s",
            baseData.attackClass,
            tostring(AttackClass)))
        return
    end
    print("  - AttackClass loaded successfully.")

    self.attackInstance = AttackClass:new(playerManager, self)
    if not self.attackInstance then
        error(string.format("AlternatingConeStrikeE001:equip - Falha ao criar instância de AttackClass '%s'.",
        baseData.attackClass))
        return
    end
    print(string.format("  - Attack instance created (Type: %s).", type(self.attackInstance)))

    print(string.format("[AlternatingConeStrikeE001:equip] '%s' fully equipped. Attack instance created.", self.name or self
        .itemBaseId))
end

--- Desequipa as Adagas Duplas.
--- Remove a instância de ataque. A remoção de stats não é mais necessária aqui.
function AlternatingConeStrikeE001:unequip()
    print(string.format("[AlternatingConeStrikeE001:unequip] Unequipping '%s'.", self.name or self.itemBaseId))

    BaseWeapon.unequip(self)

    print(string.format("[AlternatingConeStrikeE001:unequip] '%s' fully unequipped.", self.name or self.itemBaseId))
end

return AlternatingConeStrikeE001
